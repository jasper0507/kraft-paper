...
核心原则：陶土橙做光标、勾选、焦点与侧栏激活等**装饰强调**；链接 / 脚注等字色走更深的 `--accent-text-color`，保证暖纸上的对比度。正文 / 标题走独立暖色文本阶，边线统一暖灰，避免中性灰在米色纸面上发脏。
...
| 标题 | `#1c1815` | `#F5F3EC` |
| 次级文本 | `#72695e` | `#A29A8D` |
| 强调色 `--accent-color`（装饰） | `#c15f3c` | `#D97757` |
| 强调字色 `--accent-text-color` | `#a14d2e` | `#D97757` |
| 强调悬停 | `#8a4026` | `#E69373` |
| 边线 | `#ddd5ca` | `#3E3B36` |
...
- **脚注**：上标走 `--accent-text-color`；定义区名称 / 内容分层着色
- **GitHub Alerts**（`> [!NOTE]` 等）：左强调条 + 极淡底，不做圆角卡片，与引用块同属竖线语言；明暗五色成对
- **h2**：陶土细线下划（`color-mix`，旧引擎走 `rgba` 回退）
...
| 想改什么 | 动哪个变量 / 位置 |
| --- | --- |
| 换强调色 | `--accent-color`（装饰） / `--accent-text-color`（链接与脚注） / `--accent-hover-color` / `--focus-ring-color` |
...
用 [`examples/showcase.md`](examples/showcase.md)（或任意含标题 / 列表 / 任务列表 / 表格 / 代码块 / 引用 / Alerts / 高亮 / 脚注的长文），在明暗两主题下对照：
...
7. GitHub Alerts 五种类型在明暗下左边线与标题色是否成对、不做成卡片
